...
make_pairs <- function(f, positive_pairs, scale=FALSE){
  ...
  x <- array(0, dim = c(nrow(all_pairs), ncol(dat), ncol(dat), 2 ))

  cat("starting tensor build")
  #build the tensor
  for (i in 1:nrow(all_pairs)){ #sample
    if(scale){
      ...
      for(ex in 1:ncol(dat) ){ #experiment column
        x[i,ex,ex,1] <-scale_a[ex]  #    small_cels[1,i]
        x[i,ex,ex,2] <-scale_b[ex] #  small_cels[2,i]
      }
      ...
      for(ex in 1:ncol(dat) ){ #experiment column
        x[i,ex,ex,1] <-dat[ all_pairs$TF[i], ex ]  #    small_cels[1,i]
        x[1,ex,ex,2] <-dat[ all_pairs$Target[i], ex ] #  small_cels[2,i]
      }
...
if (args == "scale"){
  make_data_set("lib/normalised_data.csv", "lib/AtRegNet", "lib/affy_ATH1_array_elements-2010-12-20.txt", scale = TRUE, out = "data/scaled_arab_TF_two_channel.RDS")
} else {
  make_data_set("lib/normalised_data.csv", "lib/AtRegNet", "lib/affy_ATH1_array_elements-2010-12-20.txt", out = "data/arab_TF_two_channel.RDS")
}