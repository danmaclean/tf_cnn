...
make_pairs <- function(f, positive_pairs, scale=FALSE){
  ...
  x <- array(0, dim = c(nrow(all_pairs), 2, ncol(dat), 1 ))
...
if (args == "scale"){
  make_data_set("lib/normalised_data.csv", "lib/AtRegNet", "lib/affy_ATH1_array_elements-2010-12-20.txt", scale = TRUE, out = "data/scaled_arab_TF_one_channel.RDS")
} else {
  make_data_set("lib/normalised_data.csv", "lib/AtRegNet", "lib/affy_ATH1_array_elements-2010-12-20.txt", out = "data/arab_TF_one_channel.RDS")
}